...
const Signup  = () => {
    ...
    const [registerNewUserEmail, { isLoading, isError }] =  useEmailRegisterMutation()
    ...
    const handleSubmitEmail = async (e) => {
        ...
        try{
            await registerNewUserEmail({ role, fname, lname, email, phone, password, repassword }).unwrap()
                setFirstName('')
                setLastName('')   
                setEmail('')
                swal({
                ...
                // save email to local storage if in case user requested another otp
                // then just get the email to local storage to lessen user's time to type email again
  ...
  return (
    <Container fluid className='public'>
         <Row>
            ...
            <Col lg='6' md='6'>
                {/* sign up with email */}
                <Form className="formRegister px-4" onSubmit={handleSubmitEmail}>
                    <h3 className='form-title text-center fw-bold'>Seller Sign Up</h3>
                    <Col className='mb1'>
                        <TextField 
                            variant='standard'
                    ...
                    <Col className='mb-2'>
                        <Button className='public-btn btn py-2 btn-md w-100'>Create Account</Button>
                    </Col>
                    <Col className='d-flex justify-content-between'>
                        <span style={{fontSize: '12px', color: '#4B4A80'}} className='ms-1 mt-1'>
                            <MDBCheckbox name='flexCheck' value='' id='flexCheckDefault' label='Accept Terms and Condition and policy' />
                        </span>
                    ...
                    <Col className='d-flex justify-content-center mb-2'>
                        <small style={{fontSize: '12px', marginRight: '5px', fontWeight: 'bold'}}>Have an account?</small>
                        <Link style={{fontSize: '12px', marginRight: '5px', fontWeight: 'bold'}} to="/fatandcap/auth/login">Sign In</Link>
                    </Col>     
                </Form>
            </Col>
        </Row>      
