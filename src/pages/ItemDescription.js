...
import { addToCart, selectCurrentCart, selectPublicToken } from '../features/auth/authSlice'
import useAuth from '../hooks/useAuth'
import useTilte from '../hooks/useTitle'
import { Avatar, FormHelperText, InputLabel, useMediaQuery } from '@mui/material'
...
export const ItemDescription = ({id, size, color}) => {
    ...
    let itemId = queryParameters.get("item")
  ...
  useEffect(()=>{
   ...
   useEffect(()=>{
    ...
    const { usercomment } = useGetCustomerQuery('customerList', {
        selectFromResult: ({ data }) => ({
            usercomment: data?.entities[user]
          }),
    })
    ...
    if(product){
        const handleViewSeller = () => navigate(`/fatandcap/seller/shop/${seller?._id}`)
        ...
        <Container>
            <Row>
                <Col lg={12}>
                    ...
                    {/* need css */}
                    <div className='mb-3' style={{cursor: 'pointer'}} onClick={handleViewSeller}>
                        <span style={{color: '#4B4A80', fontWeight: 'bolder'}} className='d-flex justify-content-center align-items-center mt-4'> 
                            <Avatar src={seller?.image === "null" ? logo : seller?.image} sx={{ width: 50, height: 50, marginRight: '15px' }}/>
            ...
            <Row className='m-auto'>
                <Col md='4'>
                    <div className="product_items mt-5 py-2">
                                        ...
                                        >
                                        {
                                            comment?.map((item, key) =>{
                                                return(
                                                    ...
                                                    <SwiperSlide key={key}>
                                                    <div>
                                                        <div className='d-flex flex-row justify-content-start align-items-start mt-3'>
                                                            <Avatar src={!item?.image || usercomment?.image !== "null"  ? logo : usercomment?.image} sx={{ width: 45, height: 45, marginRight: '15px' }}/> 
                                                ...
                                                return(
                                                    ...
                                                    <div>
                                                        <div className='d-flex flex-row justify-content-center align-items-center mt-3'>
                                                            <Avatar src={usercomment?.image === "null" ? logo : usercomment?.image} sx={{ width: 45, height: 45, marginRight: '15px' }}/> 
                                                            <span className='d-flex flex-column'>
                                                               <small style={{fontSize: '12px'}}>
                                                                 {characterHide(item?.username)}
                                                                </small>
